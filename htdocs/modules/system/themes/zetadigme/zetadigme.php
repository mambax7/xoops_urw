...
class XoopsGuiZetadigme extends XoopsSystemGui
{
    ...
    function XoopsGuiZetadigme()    { $this->__construct();  }
    function validate()    {  return true; }
    function generateMenu()    {  return true; }
    function header()    {
        parent::header();
...
window.onload= function(){
	...
	//start system overview
        ...
        'text' => _AM_SYSTEM_MODULES, 'dir' => $mod->getVar('dirname', 'n'), 'menu' => $menu));
	...
	## URW ##
       $OPT[] = array(
		...
		'title'     => _THEME_URW,
		'absolute'  => 1,
		'icon'     => XOOPS_ADMINTHEME_URL . '/zetadigme/icons/urw_small.png');
	## URW ##
       $OPT[] = array(
	...
	//add OPTIONS/links for local support
        ...
        if (is_object($xoopsModule) || !empty($_GET['xoopsorgnews'])) {  return; }
        foreach ($mods as $mod) {
            $rtn = array();
            $moduleperm_handler =& xoops_gethandler('groupperm');
            $sadmin = $moduleperm_handler->checkRight('module_admin', $mod->getVar('mid'), $xoopsUser->getGroups());
